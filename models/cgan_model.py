...
class cGAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        #Define generators : G_x and G_y
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc,
                                        opt.ngf,  opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        #Define discriminators if isTrain flag is 1
        if self.isTrain:
        ...
        #define loss functions and intialize optimizers
        if self.isTrain:
